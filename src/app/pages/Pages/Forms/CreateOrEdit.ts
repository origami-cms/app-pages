import { ButtonOptions, ZenForm } from '@origami/zen';
...
export class AppPagesPagesForm extends connect(window.Origami.store)(LitElement) {
  ...
  render() {
    ...
    return html`
      ...
      <zen-form
        .values=${this.values}
        @change=${(e: Event) => this._handleChange(e.target)}
        @dirtyChange=${(e: Event) => this._handleDirtyChange(e.target)}
        @submit=${(e: Event) => this._handleSubmit(e.target)}
        saveable
      >
        <header>
          <h3>
